...
class BuhlmannParser(Parser):
    ...
    """
        toggle = kwargs.get('toggle', None)
        self.model = model
        self._structure_dict = {'headers': ['MaxN2Ins', 'MaxHeIns', 'MaxInsTotal', 'MaxBub', 'N2CP', 'HeCP', 'N2Surf', 'HeSurf', 'GFLowMax', 'GFHigh'], 'special': [
            'GFLowMaxMax', 'GF100D', 'FirstMiss', 'GFHighMax', 'OTU', 'MaxAscent'], 'dives': []}

        if(toggle == False):
            self._structure_dict = {'headers': [
                'MaxN2Ins', 'MaxHeIns', 'MaxInsTotal', 'MaxBub', 'N2CP', 'HeCP', 'N2Surf', 'HeSurf'], 'special': ['OTU', 'MaxAscent'], 'dives': []}
