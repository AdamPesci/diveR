    ...
    def write_log(self, path, message):
        ...
        if not os.path.isdir('./logs'):
            ...
            try:
                ...
                sys.stderr.write("file not accessible")
            finally:
                fw.close()
...
class ANSLoader(FileLoader):
            ...
            if (path.endswith('.ans') or path.endswith('.txt')):
                ...
                for line in rows:
                    ...
                    if (file_line == 1):
                        ...
                        data_frame[i][1] = str((float(elements[1]) * 0.30643))
                    ...
                    # End of the actual file and not the start of the file
                        ...
                        data_frame = data_frame[~np.all(
                            data_frame == 0, axis=1)]
                        gas_frame = gas_frame[~np.all(gas_frame == 0, axis=1)]
    ...
    def generate_gas_list(self, gas_data):
        """
    generate gas list from ans gas codes
    :param gas_data: dataframe:
        dataframe created by ans_loader.load. Dataframe contains times and gas codes
